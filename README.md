# Curriculum Vitae

Portafolio profesional de trabajo completo y moderno donde puedes explorar a fondo mi trayectoria, habilidades, certificaciones y todo mi historial de proyectos en detalle.

[![PORTAFOLIO](https://img.shields.io/badge/PORTAFOLIO-18181B?style=for-the-badge&logo=github&logoColor=white&labelColor=27272A&color=18181B)](https://javierfernandezvaca.dpdns.org)

---

## ¿Qué encontrarás?

- **Historial completo de proyectos**: Explora todas las aplicaciones desarrolladas para clientes de todo el mundo.
- **Cartas de recomendación**: Respaldo directo de mis clientes y colaboradores.
- **Arsenal técnico**: El stack tecnológico con el que trabajo.
- **Contacto directo**: Toda la información necesaria para iniciar nuestra próxima colaboración.
